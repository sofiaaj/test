...
#### Estimate hours in each setting
...
**Categorizing in-person and remote workers**
...
#### Idenitifying risk-takers and risk-averse respondents
...
Having categorized YouGov respondents as risk-averse or risk-takers, we can create a prediction model using variables also available in the Pulse:


**vaccine status and beliefs**: We use a series of variables relating to whether a person is vaccinated or not, if they want to be vaccinated, and their general beliefs about the vaccine, and government/health authorities:
...
We group these variables into a single one: **vacc_status
...
yougov <- yougov %>% 
  ...
  dplyr::mutate(vacc_status = case_when(
    isvax %in% c("Yes, one dose","Yes, two doses") ~ "is_vaccinated",
    wantsvax %in% c("1","2","3") ~ "wants_vaccine",
    TRUE ~ "not_want_vaccine"
  )) %>%
...
yougov %>%
  dplyr::filter(isvax != " ") %>%
  dplyr::mutate_at(c("isvax"),
                   recode,
                   "No, neither" = "0",
                   "Yes, one dose" = "1",
                   "Yes, two doses" = "1") %>%  
  group_by(date,isvax,side_effects) %>%
  dplyr::summarise(count = n())
```
...
Should we do this with an ML classification algorithm? Idk the benefits, to research!
...
```{r}
pulse_grouped <- pulse_grouped %>%
  mutate(vacc_status = case_when(
    isvax == 1 ~ "is_vaccinated",
    wantsvax %in% c(1,2) ~ "wants_vaccine",
    TRUE ~ "not_want_vaccine"),
  gov_skeptic = ifelse(gov_skeptic == 1,"Yes","No"),
  side_effects = ifelse(side_effects == 1,"Yes","No"),
  mistrust_vax = ifelse(mistrust_vax == 1,"Yes","No"))
```

```{r}
preds <- predict(risky_model,pulse_grouped,type="response")
pulse_grouped$risky_pred <- preds
```

```{r}
pulse_grouped %>%
  dplyr::group_by(vacc_status,gov_skeptic,side_effects,mistrust_vax,risky_pred) %>%
  dplyr::summarise(n())
```
